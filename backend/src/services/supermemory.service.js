...
export const updatePathMemory = async (userId, pathId, pathData) => {
  try {
    ...
    if (cachedMemoryId) {
      ...
      if (searchResults && searchResults.results && searchResults.results.length > 0) {
        ...
        for (const result of searchResults.results) {
          const content = result.memory || result.content || result.document || "";
    ...
    if (!memoryId) {
      console.log("⚠️ No existing memory found, creating new one");
    ...
    console.log("⚠️ Update failed, creating new memory");
    return await storePathMemory(userId, pathId, pathData);
  }
};
  ...
  try {
    ...
    if (cachedMemoryId) {
      ...
      try {
        ...
        if (memory && (memory.content || memory.memory)) {
...
export const clearMemoryCache = (pathId) => {
  memoryCache.delete(pathId);
  console.log(`🗑️ Cleared cache for pathId: ${pathId}`);
};
